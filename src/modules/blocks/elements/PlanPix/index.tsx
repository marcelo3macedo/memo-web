...
export default function PlanPix() {
  ...
  const { option, loading, failed, payment } = useSelector(
    (state: RootState) => state.planOptions
  );
    ...
    <Wrapper>
      <Container>
        ...
        <Info>
          ...
          {failed ? (
            <InfoErrorMessage>{t('planOptions.pix.error')}</InfoErrorMessage>
          ) : (
            <></>
          )}
          <ButtonPrimary
            content={t('planOptions.pix.payment')}
            action={paymentClick}
          />
        </Info>
