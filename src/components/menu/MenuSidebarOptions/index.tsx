...
import { PATH_ABOUT, PATH_ACCOUNT, PATH_CREATESESSION, PATH_GALLERY, PATH_INTEGRATION, PATH_MAIN, PATH_PLANS, PATH_SESSIONS } from '@services/Navigation';
  ...
  return (
    <Wrapper>
      <Content>
        ...
        { isPro ?  (<MenuSidebarOption icon="next" content={t('menu.integrations')} action={() => { navigate(PATH_INTEGRATION) } }/>) : <></> }
        <MenuSidebarOption icon="plans" content={t('menu.plans')} action={() => { navigate(PATH_PLANS) } }/>
        <MenuSidebarOption icon="users" content={t('menu.account')} action={() => { navigate(PATH_ACCOUNT) } }/>
