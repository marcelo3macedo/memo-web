...
function* open({ payload }: any) {
  const response = yield request({
    type: REQUESTER_GET,
    method: `${API_DECKS}/${payload.deck.id}`,
  });
...
function* openPublic({ payload }: any) {
  const response = yield request({
    type: REQUESTER_GET,
    method: `${API_DECKS}/${payload.deck.id}?isPublic=true`,
  });
...
function* review(data) {
  ...
  const response = yield request({
    type: REQUESTER_GET,
    method: `${API_SESSIONSFEED}/${deck.id}`,
  });
